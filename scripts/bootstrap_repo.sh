...
copy_if_missing "$ROOT_DIR/templates/ai" "$AI_ENTRY"
if [[ -f "$AI_ENTRY" ]]; then
  chmod +x "$AI_ENTRY"
fi
...
say "2. Run './ai status' to verify the daily entrypoint."
say "3. Run './ai spec <spec-id> <source-doc>' or './ai run <spec-id> <source-doc>' for a new request."
